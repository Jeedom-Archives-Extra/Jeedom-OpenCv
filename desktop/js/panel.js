...
initTableSorter();
$('.snapshot').load('index.php?v=d&modal=openCv.history&plugin=openCv&type=openCv');
				...
				for (var i in data.result) {
					if(data.result[i]['logicalId'] != "lastdetect" && data.result[i]['logicalId'] !='*'){
						$('#table_DetectInconnue tbody').append($("<tr>")
							...
							.append($("<td>").load('index.php?v=d&modal=movePlate.openCv&plugin=openCv&type=openCv&id='+data.result[i]['id']))
							.append($("<td>")
