...
log::add('openCv','debug','Lancement du script de détéction');
$JsonDetect=file_get_contents('php://input');
log::add('openCv','debug',$JsonDetect);
$Detect=json_decode($JsonDetect, true);
openCv::SendLastSnap($Detect);
//$Detect["uuid"];
//$Detect["camera_id"];
//$Detect["site_id"];
//$Detect["img_width"];
...
foreach($Detect["results"] as $Results){
		...
		if(is_array($CmdPlates)){
			foreach($CmdPlates as $CmdPlate){
				if (is_object($CmdPlate)) {
					log::add('openCv','debug','La plaque d\'immatriculation  '.$Plate["plate"].' a ete détécté avec la confidence '.$Plate["confidence"]);
					$PlateConfigure=true;
	...
	if(!$PlateConfigure && config::byKey('inconnue','openCv')){
		$Equipement = openCv::addEquipement('Plaques détectées inconnu','inconnu');
		$CmdPlate=openCv::addCommande($Equipement,$Results["plate"],$Results["plate"]);
